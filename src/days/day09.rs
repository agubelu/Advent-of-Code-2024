...
fn compact_v1(layout: &[Block]) -> i64 {
    ...
    while file_ptr > space_ptr {
        ...
        while layout[space_ptr].ident != -1 { space_ptr += 1 }
        while layout[file_ptr].ident == -1 { file_ptr -= 1 }
    }
...
fn compact_v2(mut layout: Vec<Block>) -> i64 {
    ...
    while file_ptr > 0 {
        ...
        let space = (0..file_ptr).find(|&i| layout[i].ident == -1 && layout[i].size >= block_to_move.size);
        if let Some(space_ix) = space {
            move_block(&mut layout, file_ptr, space_ix);

            if layout[space_ix + 1].ident == -1 {
...
fn move_block(layout: &mut Vec<Block>, source_ix: usize, target_ix: usize) {
    ...
    match space_size.cmp(&file_size) {
        ...
        Greater => {
            ...
            layout[source_ix] = Block{ size: file_size, ident: -1 };
            layout.insert(target_ix + 1, Block{ size: leftover_space, ident: -1 });
        },
        Less => {
...
fn parse_layout(input: &str) -> Vec<Block> {
    input.trim_end().char_indices()
         .map(|(i, ch)| {
             ...
             let ident = if i % 2 == 0 { i as i64 / 2 } else { -1 };
             Block { size, ident }
         }).collect()
}
